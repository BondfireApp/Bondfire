...
export default function PublicPage() {
  ...
  React.useEffect(() => {
    ...
    loadPublicPage(publicSlug)
      ...
      .catch((error) => {
        if (alive) setState({ loading: false, data: null, error: String(error?.message || error) });
      });
    return () => { alive = false; };
  }, [publicSlug]);
  ...
  // Preserve the mature mutual-aid renderer for existing organizations and for
  // errors. It performs its own fetch/error handling and remains the safe default.
