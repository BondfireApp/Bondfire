...
export const WORK_HELP = {
  ...
  treasury: 'Create and name your own funds, then record income, expenses, reimbursements or transfers. Amounts use two decimal places. Pending approvals, unpaid reimbursements, drafts and void records do not affect balances. Public transparency is one organization-wide switch, off by default. When on, all transactions and balances update automatically on your Organization Page. Use safe public transaction names; private notes, identities and attachments stay private. Archiving does not hide entries.',
};
...
export default function WorkPage({ page }) {
  ...
  return <main className="work-page">
    ...
    {session && can('view') && <>
      ...
      {editor && <WorkRecordForm transparency={!!session.context.transparency?.enabled} key={`${type}:${editor.id || 'new'}`} type={type} initial={editor} all={all} members={members} catalog={catalog} settings={caseSettings} me={session.context.userId} busy={busy} onSave={save} onCancel={() => setEditor(null)} />}
...
function friendlyError(e) {
  const messages = { COMPLETE_LEDGER_REQUIRED: 'The complete Treasury must be included. Refresh on a device with access to every fund and transaction.', TRANSPARENCY_CHANGED: 'The organization transparency setting changed. Refresh before saving.', SAFE_PUBLIC_FIELDS_REQUIRED: 'Add a public transaction name without personal information before saving.', PUBLIC_LEDGER_FIELD_LOCKED: 'Record an adjustment instead of changing a published opening balance, currency or transaction fund.', PRIVATE_REVISION_CONFLICT: 'This record changed elsewhere. Your form is still open; refresh and compare before saving.', PRIVATE_MODE_NOT_READY: 'Enable encrypted storage in Settings → Security before using these modules.', MODULE_DISABLED: 'This module is disabled. Enable it in Build.', KEY_RECIPIENT_MISSING: 'Device access changed. Refresh this page before saving.', NOT_FOUND: 'The record or a linked record is unavailable.', INSUFFICIENT_ROLE: 'Your role does not allow that action.' };
