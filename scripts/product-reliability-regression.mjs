...
const checks = [
  ...
  ["People is a selectable module rather than Core", "src/platform/moduleRegistry.js", /const CORE_MODULE_IDS = Object\.freeze\(\[\"public-site\"\]\);[\s\S]*const LIVE_MODULES = \[[\s\S]*id: \"people\"[\s\S]*tier: \"live\"/],
  ["Newsletter is a selectable module", "src/platform/moduleRegistry.js", /id: \"newsletter\"[\s\S]*routeBase: \"settings\?tab=newsletter\"[\s\S]*tier: \"live\"/],
  ["People and Newsletter have builder assets", "src/components/BuildModules.jsx", /people: \"\/logos\/people\.svg\"[\s\S]*newsletter: \"\/logos\/newsletter\.svg\"/],
  ["People and Newsletter are gated module navigation", "src/components/AppHeader.jsx", /const moduleItems = base[\s\S]*label: \"People\"[\s\S]*moduleId: \"people\"[\s\S]*label: \"Newsletter\"[\s\S]*moduleId: \"newsletter\"/],
  ["dashboard metrics obey People and Newsletter enablement", "src/pages/Overview.jsx", /mk\(\"people\", \"People\", \"\/logos\/people\.svg\", countsNormalized\.people, \"in roster\", \"people\", \"people\"\)[\s\S]*mk\(\"subsTotal\", \"New Subs\", \"\/logos\/newsletter\.svg\", countsNormalized\.subsTotal, \"total\", \"settings\?tab=newsletter\", \"newsletter\"\)/],
  ["dashboard panels only render enabled summary modules", "src/pages/Overview.jsx", /const visiblePanels = \[[\s\S]*moduleId: \"intake\"[\s\S]*moduleId: \"meetings\"[\s\S]*moduleId: \"inventory\"[\s\S]*moduleId: \"needs\"[\s\S]*moduleId: \"pledges\"[\s\S]*\]\.filter\(\(item\) => isDashboardModuleEnabled\(item\.moduleId\)\);/],
  ["dashboard retains summary panels while module configuration is unresolved", "src/pages/Overview.jsx", /const isDashboardModuleEnabled = \(moduleId\) =>[\s\S]*enabledModules === null \|\| enabledModules\.has\(moduleId\);/],
  ["existing module configurations retain former universal People and Newsletter access until resaved", "functions/api/orgs/[orgId]/modules.js", /MODULE_CONFIG_SCHEMA_VERSION = 2[\s\S]*LEGACY_OPTIONAL_MODULE_IDS = Object\.freeze\(\[\"people\", \"newsletter\"\]\)[\s\S]*includeLegacyOptionalModules: isLegacyModuleConfig\(row\)/],
  ["new module configurations persist their explicit schema version", "db/schema.sql", /org_module_configs[\s\S]*module_schema_version INTEGER NOT NULL DEFAULT 2/],
  ["module checks retain the public site as Core", "functions/api/_lib/orgModules.js", /CORE_MODULE_IDS = Object\.freeze\(\[\"public-site\"\]\)[\s\S]*return new Set\(\[[\s\S]*\.\.\.CORE_MODULE_IDS/],
