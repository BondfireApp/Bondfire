...
const checks = [
  ...
  ["dashboard panels only render enabled summary modules", "src/pages/Overview.jsx", /const visiblePanels = \[[\s\S]*moduleId: \"intake\"[\s\S]*moduleId: \"meetings\"[\s\S]*moduleId: \"inventory\"[\s\S]*moduleId: \"needs\"[\s\S]*moduleId: \"pledges\"[\s\S]*\]\.filter\(\(item\) => enabledModules\?\.has\(item\.moduleId\)\);/],
